...
<body id="tmpl_popup">	
		...
		<h3>Dettaglio incaricato "${incaricato.id}"</h3>
		
		<form id="gestioneReferente" action="/tolomeobinj/glup/GestioneIncaricato" method="post">
				
		<fieldset>
			<legend>Dati Incaricato</legend>											
				<div>
					<span class="tmpl_form_etichetta">Nome : </span>
					<span class="tmpl_form_fixtext">${incaricato.nome}</span>
				</div>
				<div>
					<span class="tmpl_form_etichetta">Cognome : </span>
					<span class="tmpl_form_fixtext">${incaricato.cognome}</span>
				</div>				
		        <div>
		            <span class="tmpl_form_etichetta">N. ufficio : </span>
		            <span class="tmpl_form_fixtext"><c:out value="${incaricato.numTel1}" default="- - - - - - - - - -" /></span>		            
		        </div>
		        <div>
		            <span class="tmpl_form_etichetta">N. cellulare : </span>
		            <span class="tmpl_form_fixtext"><c:out value="${incaricato.numTel2}" default="- - - - - - - - - -" /></span>		            
		        </div>
		        <div>
		            <span class="tmpl_form_etichetta">N. fax : </span>
		            <span class="tmpl_form_fixtext"><c:out value="${incaricato.fax}" default="- - - - - - - - - -" /></span>		            
		        </div>
		        <div>
		            <span class="tmpl_form_etichetta">Email : </span>
		            <c:choose>
			            <c:when test="${empty incaricato.email}">
			            	<span class="tmpl_form_fixtext">- - - - - - - - - -</span>
			            </c:when>
			            <c:otherwise>
			            	<a href="mailto:${incaricato.email}"><span class="tmpl_form_fixtext">${incaricato.email}</span></a>
			            </c:otherwise>
		            </c:choose>			            
		        </div>		        		        				        
		</fieldset>								
