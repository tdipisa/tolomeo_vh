...
<body onload="init();" id="tmpl_popup">
	...
	<form action="/tolomeobinj/esempiobase/GestioneEsempioBaseServlet" method="post">
		...
		<legend>Sinistro</legend>
			<div>
	            ...
	            <select id="anno" name="anno">
	            	<c:choose>
						<c:when test="${!empty sinistro}" >
							<option selected="selected" value="${sinistro.anno}">${sinistro.anno}</option>
							<option value="<%= oggi.getYear() %>"><%= oggi.getYear() %></option>
						</c:when>
						<c:otherwise>
							<option selected="selected" value="<%= oggi.getYear() %>"><%= oggi.getYear() %></option>
						</c:otherwise>
					</c:choose>
					<% for(int i=1; i<3; i++) { %>
	            		<option value="<%= oggi.getYear()-i %>"><%= oggi.getYear()-i %></option>
	            	<% } %>
	            </select>
	        </div>
	      	<div>
	            <label for="nSinistro">Numero</label>
	            <input type="text" name="numeroSinistro" value="${sinistro.NSinistro}" id="numeroSinistro" title="Numero sinistro stradale" size="9" maxlength="8" accesskey="" />
	        </div>
	</fieldset>
