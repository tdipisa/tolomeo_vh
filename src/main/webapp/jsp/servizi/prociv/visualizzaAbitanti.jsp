...
<% 
	List<PuntoAnagrafe> abitanti = (ArrayList<PuntoAnagrafe>)request.getAttribute("abitanti"); 
...
<%@page import="it.prato.comune.sit.PuntoAnagrafe"%><style type="text/css">
...
<body onload="mapOperation();" id="tmpl_popup">
	...
	<form action="CreaReportAnagrafeServlet">
		<div class="tmpl_form_hidden">	
			<input type="hidden" name="IDTPN" value="<%= key %>" />
			<input type="hidden" name="codTPN" value="<%= codTPN %>" />
			<input type="hidden" name="command" value="famiglie" />
		</div>
		<div class="tmpl_stampa_doc">
			<input id="stampaXls" type="submit" accesskey="X" title="Crea documento XLS" value="crea xls" name="X"/>
		</div>
		<div class="tmpl_bottoni">
			<input class="generico" name="indietro" value="indietro" title="Indietro" type="button" onclick="javascript:history.back();" />
		</div>
	</form>
	...
	<caption>Famiglie residenti</caption>
		...
		<tbody>
			<c:forEach items="${abitanti}" var="row" varStatus="rowStatus">
				<c:choose>
