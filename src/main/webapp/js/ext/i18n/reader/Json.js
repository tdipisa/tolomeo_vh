Ext.define('Ext.i18n.reader.Json', {
    ...
    getKeys: function (obj, parent) {
            ...
            for (key in obj){
                if(obj.hasOwnProperty(key)){
                    ...
                    if(Ext.isObject(obj[key])){
                       r1 = traverse(obj[key], p);
                       retVal = retVal.concat(r1)
                    } else {
                    	retVal.push(p);
                    }
        ...
        for (key in obj) {
            if (obj.hasOwnProperty(key)) {
                if(Ext.isObject(obj[key])){
                	keys = keys.concat(traverse(obj[key], key));
                }else{
                    keys.push(key);
                }
    ...
    /* ORIGINALE 
     * NON PERMETTEVA DI RECUPERARE CHIAVI DEL TIPO (recuperava solo la prima) 
     * server.title
     * server.altrachiave
    getKeys: function (obj, parent) {
        var me = this, key, keys = [];
