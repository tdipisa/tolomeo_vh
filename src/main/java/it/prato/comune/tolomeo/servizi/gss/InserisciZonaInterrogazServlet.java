...
public class InserisciZonaInterrogazServlet extends TolomeoServlet {    
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, java.io.IOException {
		...
		if (StringUtils.isEmpty(codTPN)) {
            ...
            String errMsg = "Parametro nome non valorizzato";
			logger.error(errMsg);
			addMessaggio(request, Messaggio.ERRORE, errMsg);
			forward(TolomeoServlet.ERROR_PAGE, request, response);
            return;
		...
		try {
			...
			if (geoOp.equals(Parametri.digitizeOperationInsert)) {
			    PoligonoZonaInterrog zona =(PoligonoZonaInterrog) JSGeometry.jsGeometryToOggettoTerritorio(comunePO, geoCoord, logger);
			    zona.setNome(nome); 
    			
    			layer.appendFeature(zona);
    			
			} else if (geoOp.equals(Parametri.operationUpdateAlfa)) {
			    PoligonoZonaInterrog zona = (PoligonoZonaInterrog) layer.cercaIDTPN(idTPN);
			    zona.setNome(nome);   
                layer.modifyFeature(zona);
			}
			...
			if (e.getSQLErrorCode()==1) {
				...
				String errMsg = "Identificativo zona interrogazione duplicato!";
				logger.error(errMsg,e);
				addMessaggio(request, Messaggio.ERRORE, errMsg);
		...
		addMessaggio(request, Messaggio.INFORMAZIONE, "Zona Interrogazione inserita correttamente");
