...
/**
 * Servet che riceve la richiesta di visualizzazione della statistica di un plesso scuola primaria e la dispaccia alla jsp idonea.
 * @author Mattia Gennari
 *
 */
public class VisualizzaPlessoPrimariaServlet extends TolomeoServlet {    

	private static final long serialVersionUID = 4553276188184045350L;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, java.io.IOException {
		...
		LayerPlessiPrimarie plessiPrimarie = (LayerPlessiPrimarie) comunePO.getLayerByCodTPN(codTPN);
		LayerAnagrafe anagrafe = LayerAnagrafe.getInstance(comunePO); 

		// Cerco plesso selezionato
		PoligonoTerritorio plessoPrimaria = (PoligonoTerritorio) plessiPrimarie.cercaIDTPN(key);

		if(plessoPrimaria == null){
		...
		try {
			...
			// Carico le statistiche dei bambini associati al plesso materna associato
			statisticaPlesso = service.getStatisticaPlessoPrimaria(plessoPrimaria);
			statisticaPlesso.setNomeLayer(plessiPrimarie.getNome());

			jsonGeometryToExtent = plessoPrimaria.getJSGeometry();
		...
		request.setAttribute("statisticaPrimaria", statisticaPlesso);
		request.setAttribute("plessoPrimaria", plessoPrimaria);
	...
	protected String getDefaultForward() {
		return "/jsp/servizi/pubblicaIstruzione/visualizzaStatisticaPlessoPrimaria.jsp";
	}
}
