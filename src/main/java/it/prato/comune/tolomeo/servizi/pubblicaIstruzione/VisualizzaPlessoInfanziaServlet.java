...
/**
 * Servet che riceve la richiesta di visualizzazione della statistica di un plesso scuola dell'infanzia e la dispaccia alla jsp idonea.
 * @author Mattia Gennari
 *
 */
public class VisualizzaPlessoInfanziaServlet extends TolomeoServlet {    

	private static final long serialVersionUID = 2466047582171217375L;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, java.io.IOException {
		...
		LayerPlessiInfanzia plessiInfanzia = (LayerPlessiInfanzia) comunePO.getLayerByCodTPN(codTPN);
		LayerAnagrafe anagrafe = LayerAnagrafe.getInstance(comunePO); 

		// Cerco plesso selezionato
		PoligonoTerritorio plessoInfanzia = (PoligonoTerritorio) plessiInfanzia.cercaIDTPN(key);

		if(plessoInfanzia == null){
		...
		try {
			...
			// Carico le statistiche dei bambini associati al plesso infanzia associato
			statisticaPlesso = service.getStatisticaPlessoInfanzia(plessoInfanzia);
			statisticaPlesso.setNomeLayer(plessiInfanzia.getNome());

			jsonGeometryToExtent = plessoInfanzia.getJSGeometry();
		...
		request.setAttribute("statisticaInfanzia",statisticaPlesso);
		request.setAttribute("plessoInfanzia",plessoInfanzia);
	...
	protected String getDefaultForward() {
		return "/jsp/servizi/pubblicaIstruzione/visualizzaStatisticaPlessoInfanzia.jsp";
	}
}
