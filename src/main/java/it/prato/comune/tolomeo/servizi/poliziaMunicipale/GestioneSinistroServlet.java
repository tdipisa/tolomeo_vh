...
public class GestioneSinistroServlet extends TolomeoServlet {    
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, java.io.IOException {
        ...
        String nSinistro = request.getParameter("numeroSinistro");
		...
		if (codTPN==null) {
			addMessaggio(request, Messaggio.ERRORE, "Chiave univoca layer non valorizzata");
            forward(TolomeoServlet.ERROR_PAGE, request, response);
            return;
		}
		if (StringUtils.isEmpty(geoOp)) {
		...
		if (sinistri==null) {
			String errMsg = "Il layer " + codTPN + " non ï¿½ stato istanziato correttamente";
			logger.error(errMsg);
			addMessaggio(request, Messaggio.ERRORE, errMsg);
			forward(TolomeoServlet.ERROR_PAGE, request, response);
            return;
		...
		try {
			...
			//setto il sinistro
			sinistro.setAnno(sinistri.anno);
			if (StringUtils.isEmpty(nSinistro)) {
				sinistro.setNSinistro(null);
			} else {
				try {
					sinistro.setNSinistro(Integer.parseInt(nSinistro));
				} catch (NumberFormatException e) {
		        	addMessaggio(request, Messaggio.ERRORE, "Formato numero sinistro non valido");
		        	errors = true;
		        }
			}
			
			//controllo validita dei parametri recuperati
			if (StringUtils.isEmpty(nSinistro)) {
	    	...
	    	if (errors) {
		    	...
		    	forward("/poliziaMunicipale/CaricaDatiSinistroServlet", request, response);
		        return;
	    	} else {
	    		if (geoOp.equals(Parametri.digitizeOperationInsert)) {
					sinistri.appendFeature(sinistro);
	    		} else if (geoOp.equals(Parametri.operationUpdateAlfa)) {
	    			sinistri.modifyFeature(sinistro);
	    		}
	    	}

		} catch (SITIllegalAttributeException e) {

			String errMsg = "Si e' verificata un'eccezione durante l'aggiornamento dei dati";
			...
			if (e.getCause().getMessage().equalsIgnoreCase("ORA-00001: violata restrizione di unicitï¿½ (SIT.ANNO_SINISTRO)\n")) {
				
				String errMsg = "Sinistro " + nSinistro + " gia' presente nell'anno " + sinistri.anno;
				logger.error(errMsg,e);
				addMessaggio(request, Messaggio.ERRORE, errMsg);
		...
		request.setAttribute("sinistro",sinistro);
		forward("/poliziaMunicipale/VisualizzaSinistroServlet", request, response);
	}
